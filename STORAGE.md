# Where FoxCull Codex stores things

FoxCull Codex uses `_FoxCullCodex` for its per-drive catalog/cache/trash. The
stable source app uses `_FoxCull`; the two are intentionally separate.
