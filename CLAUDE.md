...
that as a tampered app and reports **"FoxCull is damaged and can't be opened"** —
...
4. **Do not delete the "Verify macOS bundle signature" step** in
   ...
   before users do.
5. The **ffmpeg sidecar** in `Contents/MacOS/ffmpeg` is nested code: it must
   itself be validly signed or it invalidates the whole bundle. The macOS build
   pulls a Developer ID-signed ffmpeg from martin-riedl.de — if you ever swap
   that source, verify the replacement is signed (`codesign -dv` on it) or the
   app will not launch.
...
```bash
codesign --verify --deep --strict --verbose=2 /Applications/FoxCull.app
```
...
| `docs/EVENTS-MOVES-AND-RELINK.md` | **How metadata survives the real world.** Events, in-app moves, and the missing-file/relink pass: happy paths, adversarial cases and the places they genuinely fail. Also the refresh inventory, the proposed external-agent CLI adapter, and the proposed Trash rework. Read before changing any of them. |
