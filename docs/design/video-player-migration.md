...
**Status as of 2026-07-21, 22:15 IST: DECIDED — Architecture C** (a WebCodecs
scrub engine inside the webview), chosen after a ground-up review (§10) and a
GREEN feasibility probe measured on the real hardware (§11). The libmpv work is
archived, not deleted: branches `archive/libmpv-A-mpv-in-front` (the working
in-front build) and `archive/libmpv-B-transparent-webview` (the proven-impossible
transparent approach), both pushed; `feat/libmpv-video` points at the same tip
as archive B. §§4–8 are the libmpv record — kept verbatim because the wrong
...
## 10. The ground-up review (2026-07-21, Fable) — why none of §8's options won

§8 posed a three-way choice. A review with fresh eyes found the question itself
carried a hidden assumption: **that instant seeking requires a native decoder,
and therefore a native window.** It does not. Three facts dissolve it:

1. **Playback was never broken.** §1 says it: the webview plays these files
   fine. The 4K60 HEVC Main 10 clips already hardware-decode *inside* WebView2
   every time one plays in v1.1.0. The decoder was behind the glass all along.
2. **Why `<video>` seeking is bad:** every `currentTime` set in Chromium is a
   *precise* seek — pipeline flush, decode from the previous keyframe to the
   exact frame, audio re-sync. Thirty of those during one drag lags the cursor.
   The spec's cheap alternative, `fastSeek()` (nearest keyframe), is
   **not implemented in Chromium** — Safari/Firefox only. The element cannot be
   made to scrub like mpv, ever.
3. **Why mpv feels instant:** during a drag it does *keyframe-only* seeks on a
   decoder that stays hot, and only pays for the precise decode on release. A
   seek **policy**, not special hardware access.

Since Chromium 107/108, that policy is implementable inside the page:
**WebCodecs** `VideoDecoder` is a raw, persistent, hardware-backed (D3D11VA)
decoder — HEVC Main and Main 10 included — fed encoded samples directly, no
`<video>` pipeline involved. The layering problem that consumed the whole libmpv
effort simply does not exist: the pixels are DOM pixels.

**Architecture C, concretely:**

- **Playback stays `<video>`.** Audio, A/V sync, autoplay, EditStudio: all
  untouched. It was never the problem.
- **On focusing a video:** parse the MP4 `moov` box only (mp4box.js; a few MB
  even on a multi-GB camera file, because `appendBuffer`'s return value jumps
  clean over the mdat). The sample tables yield every frame's byte offset/size
  and the full keyframe list — **no prep, no cache, milliseconds.** This is
  what the sprite system was approximating with 40 pre-extracted JPEGs.
- **While dragging:** a canvas covers the video; cursor time → nearest keyframe
  ≤ t → `read_file_range` those bytes → decode on the hot `VideoDecoder` →
  paint. Full-resolution frames locked to the cursor.
- **On release:** decode forward from that keyframe to the exact frame (bounded
  by one GOP — the same work mpv's precise seek does), set
  `video.currentTime = t` **once**, swap the canvas out on `seeked`.
- **Fallback:** feature-detect (`isConfigSupported`) per file; anything
  unsupported falls back to today's behaviour. Sprites stay for grid tiles
  (a decoder per tile is not a thing), exactly per the owner's scoping in §2.

Compared to §8: Option 1's freeze-frames, reserved strips and `overlayOpen`
plumbing all existed only because the pixels lived outside the DOM — deleted,
not mitigated. Option 3 (fork wry) becomes pointless. Option 2's sprite lever
stays open for grid, unchanged.

## 11. The feasibility probe — GREEN (measured 2026-07-21, dev machine)

`src/lib/scrub-probe.ts`, dev-only, auto-runs when `VITE_SCRUB_PROBE` names a
file; report persisted via `scrub_probe_report` to
`%TEMP%/foxcull-scrub-probe.json`. Run against a real Osmo Pocket 3 clip
(`DJI_20260622190543_0628_D.MP4`, 3840×2160 HEVC Main 10 `hvc1.2.4.H150`,
546 s, 32 737 samples) **on the P: spinning HDD**, inside the shipped WebView2
(Edg/150):

| Measurement | Result | Meaning |
|---|---|---|
| `isConfigSupported`, prefer-hardware | **true** | D3D11VA path exists in WebView2 |
| moov-only parse | 316 ms, 4.9 MB read | index of all 32 737 samples, no prep |
| Keyframe cadence | 1092 keys, GOP ≈ 30 frames | a keyframe every 0.5 s of timeline — dense scrub grid for free |
| Hot decode, 4K keyframe | **16–19 ms** (34 ms first = warm-up) | the mpv-feel number |
| Sample read off HDD | 16–47 ms | overlaps with decode when pipelined |
| Canvas paint + pixel readback | real colour | 10-bit → display conversion works end-to-end |

≈35–50 ms per full-res 4K frame end-to-end, i.e. **20–30 scrub frames per
second at native resolution with zero preparation** — versus ~1 s per frame for
the sprite builder (§3), which also needed a build per clip and produced
low-res stand-ins. Precise seek on release decodes ≤ 30 frames (one GOP).

Probe scaffolding that stays as engine foundation: `read_file_range` (binary
IPC, `tauri::ipc::Response`), the mp4box moov-parse pattern, the hvcC/avcC
`description` extraction, and the rlib `crate-type` fix that makes local
`tauri dev` link (brought to `main` from the archive branch).

## 12. Open items (updated for Architecture C)
...
- **In/out points don't take effect** in the native path. Keys are wired and the
  ...
  move to Edit entirely, which would moot it. *(Moot under Architecture C — the
  `<video>` element and its key handling never leave.)*
- ~~Fullscreen (F) positioning of the native surface~~ — moot; no native surface.
...
- ~~macOS NSView embedding~~ / ~~CI bundling of `libmpv-2.dll`~~ — moot;
  Architecture C ships nothing native. macOS gets the same engine through
  WKWebView (WebCodecs + HEVC — verify with the same probe when a Mac is at
  hand; the probe is reusable as-is).
- **MOV/H.264 phone files:** mp4box.js parses MOV; H.264 needs the avcC branch
  of `codecDescription` (already written). Verify with a phone clip during
  engine bring-up.
